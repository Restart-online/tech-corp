...
?><span class="tfl__link-container">
    ...
    ?><span>
        <a href="#" class="<?=$arResult['SETTINGS'][Options::LIST_LINK_CLASS]?> tfl__link"><?=Loc::getMessage('tfl__choose')?></a>
    </span>
</span><?php
...
<div class="tfl-define-popup" style="display:none;"><div class="tfl-define-popup__text"><?=$arResult['CONFIRM_POPUP_TEXT']?></div>
    <div class="tfl-define-popup__buttons" style="border-radius: 0 0 <?=intval($arResult['SETTINGS']['TF_LOCATION_CONFIRM_POPUP_RADIUS'])?>px <?=intval($arResult['SETTINGS']['TF_LOCATION_CONFIRM_POPUP_RADIUS'])?>px">
        <?php if (strlen($arResult['CITY_ID'])): ?>
            <a href="#" class="tfl-define-popup__button tfl-define-popup__main tfl-define-popup__yes"><?=Loc::getMessage('tfl__yes')?></a>
            <a href="#" class="tfl-define-popup__button tfl-define-popup__second tfl-define-popup__list"><?=Loc::getMessage('tfl__list')?></a>
        <? else: ?>
            <a href="#" class="tfl-define-popup__button tfl-define-popup__main tfl-define-popup__list"><?=Loc::getMessage('tfl__list')?></a>
            <a href="#" class="tfl-define-popup__button tfl-define-popup__second tfl-define-popup__yes"><?=Loc::getMessage('tfl__close')?></a>
        <?php endif;?>
    </div>
    <div class="tfl-popup__close-container"><div class="tfl-popup__close"></div></div>
</div>
...
if (typeof BX != 'undefined')
{
    ...
    <?php if (!Options::isOrderSetTemplate() && Options::isOrderSetLocation()): ?>
        ...
        BX.addCustomEvent("onAjaxSuccess" , function(data) {
            ...
            if (typeof TFLocation !== 'undefined')
            {
                var callback = function(responce)
                {
                    if (responce.hasOwnProperty('status') && responce.status === 'success')
                    {
                        TFLocation.updateLinks(responce.location_id, responce.location_name);
                    }
                }
