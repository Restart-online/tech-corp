		...
		case 'TILE':
			...
			foreach ($arResult['SECTIONS'] as &$arSection)
			{
				...
				if (false === $arSection['PICTURE'])
					...
					$checked = (isset($arParams['SECTION_CODE_TRUE']) && $arParams['SECTION_CODE_TRUE'] == $arSection['CODE'] ? 'checked' : '') ;

				?>

				<label id="<? echo $this->GetEditAreaId($arSection['ID']); ?>" class="badge__item">
					<input type="radio" name="year" class="badge__input" <?=$checked?> value="<?=$k?>" onchange="location = '<? echo $arSection['SECTION_PAGE_URL']; ?>';">
					<span class="badge__label"><? echo $arSection['NAME']; ?></span>
				</label>
				<?
			}
			unset($arSection);
