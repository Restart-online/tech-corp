...
<?
	...
	foreach ($arResult["QUESTIONS"] as $FIELD_SID => $arQuestion)
	{
		...
		?>
			<label class="computer-form__checkbox custom-checkbox">
				<input type="checkbox" name="form_checkbox_<?=$FIELD_SID?>[]" value="<?=$arQuestion['STRUCTURE'][0]['ID']?>" id="callback-footer__checkbox_<?=$arQuestion['STRUCTURE'][0]['ID']?>" <?=$arQuestion['STRUCTURE'][0]['FIELD_PARAM']?> class="custom-checkbox__input">
	...
	?>
		<label class="computer-form__item field field--dark">
...
<div class="computer-form__button">
	<button type="submit" onclick="this.form.recaptcha_token.value = window.recaptcha.getToken()"  class="g-recaptcha button button--blue" <?=(intval($arResult["F_RIGHT"]) < 10 ? "disabled=\"disabled\"" : "");?> value="<?=htmlspecialcharsbx(trim($arResult["arForm"]["BUTTON"]) == '' ? GetMessage("FORM_ADD") : $arResult["arForm"]["BUTTON"]);?>">Оставить заявку</button>
</div>
