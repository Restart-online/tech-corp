...
<?
	...
	foreach ($arResult["QUESTIONS"] as $FIELD_SID => $arQuestion)
	{
		...
		?>
			<div class="callback-footer__row">
				...
				<label for="callback-footer__checkbox_<?=$arQuestion['STRUCTURE'][0]['ID']?>" class="callback-footer__agree"><span><?=$arQuestion['CAPTION']?></span><?=$arQuestion['STRUCTURE'][0]['MESSAGE']?>
				</label>
			</div>
		<?	
		...
		?>
			<input id="<?=$arQuestion["CAPTION"]?>_<?=$arQuestion['STRUCTURE'][0]['ID']?>" data-required="" type="hidden" name="form_hidden_<?=$arQuestion['STRUCTURE'][0]['ID']?>" value="<?=$value?>" <?=$arQuestion['STRUCTURE'][0]['FIELD_PARAM']?>>
		<?
	...
	?>
	<div class="callback-footer__row">
<button type="submit" name="web_form_submit" class="rectangle_btn callback-footer__submit" onclick="this.form.recaptcha_token.value = window.recaptcha.getToken()">Отправить</button>
</div>	
<!--
<p>
